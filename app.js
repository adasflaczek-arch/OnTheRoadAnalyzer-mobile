...
// Only use showOpenFilePicker on desktop (fine-pointer devices). On phones
// and tablets the legacy <input type="file" accept="*/*"> is dramatically
// more reliable — Android's Files app picker has been flaky with
// showOpenFilePicker (CSVs grayed out, type-filter errors) across builds.
...
elBtnImport.addEventListener('click', async () => {
  // Fast path: legacy input on any touch device, regardless of API support.
  if (isTouchDevice() || !('showOpenFilePicker' in window)) {
    elFileInput.click();
    return;
  }
  // Desktop: try the modern picker, with legacy fallback on any error.
  let handles;
  try {
...
elCalApply.addEventListener('click', () => {
  ...
  // null-safe snapshot of the current cal for diagnostic flash
  const oldCal = s.tpsCal ? { ...s.tpsCal } : { closed: NaN, wot: NaN, ccw: false };
  const oldPct = (Number.isFinite(peakRaw) && s.tpsCal)
    ? calibrateTps(peakRaw, oldCal.closed, oldCal.wot, oldCal.ccw)
    : NaN;

  // ---- replace tpsCal with a fresh object ----
  // BUGFIX: the previous version read s.tpsCal.ccw inline, which threw when
  // s.tpsCal was null (right after a WIPE / RESET) — silently aborting Apply.
  ...
  try {
    ...
    const oldStr = Number.isFinite(oldCal.closed) ? `${oldCal.closed}/${oldCal.wot}` : 'DEG';
    setStatus(`CAL ${oldStr} → ${s.tpsCal.closed}/${s.tpsCal.wot}${newCcw?' ccw':''}  peak ${fmt(oldPct)}→${fmt(newPct)}`);
  } catch (e) {
...
// INIT
// ============================================================
const APP_BUILD = 'v16-legacy-picker-on-mobile';
window.addEventListener('DOMContentLoaded', () => {
  loadSettings();
  syncSettingsInputs();
  buildPlots();
